...
def ShowRSI(tickerDf):   
    # RSI Indicator
    st.header('**RSI Indicator**')    
    plt.style.use('fivethirtyeight')    
    
    df=tickerDf['Close'].diff(1) # Minus Data and previous Data
    df.dropna()
    up=df.copy()
    down=df.copy()
    up[up<0]=0
...
def MACDSignal(signal,ColName):
    buy=[]
    sell=[]
    f = -1
    for i in range(0 , len(signal)):
        if signal[ColName][i] > signal['signal line'][i]:
            sell.append(np.nan)
            if f != 1:
                ...
                buy.append(np.nan)
        elif signal[ColName][i] < signal['signal line'][i]:
            buy.append(np.nan)
            if f != 0:
...
def ShowMACD(tickerDf):  

    st.header('**MACD Indicator**')   
    plt.style.use('fivethirtyeight')
    myrange=range(1,101)
    flen=st.select_slider ("Choose your fast MACD Length",options=myrange,value=12, key='1')    
    slen=st.select_slider ("Choose your slow MACD Length",options=myrange,value=26, key='2')  
    signalLen=st.select_slider ("Choose your slow Moving Signal Length",options=myrange,value=9)  
    chkShowMACDSignal = st.checkbox("Show Buy & Sell Signal " , key='1')

    shortEMA =tickerDf.Close.ewm(span=flen , adjust=False).mean()
    longEMA =tickerDf.Close.ewm(span=slen , adjust=False).mean()
    MACD = shortEMA - longEMA
    signal = MACD.ewm(span=signalLen , adjust = False).mean()
    

    
    
    plt.figure(figsize=(16,8))
    plt.plot(tickerDf.index , MACD , label='MACD' , color='red' , alpha=0.5)
    plt.plot(tickerDf.index , signal , label='Signal' , color='blue' , alpha=0.5)
    ...
    if chkShowMACDSignal:
         MAA= MACDSignal(tickerDf,'MACD')
         tickerDf['Buy_Signal'] = MAA[0]
         tickerDf['Sell_Signal'] = MAA[1]
         plt.clf()
         plt.scatter(tickerDf.index , tickerDf['Buy_Signal'] , color='green', label='BUY' , marker='^',s=80)
         plt.scatter(tickerDf.index , tickerDf['Sell_Signal'] , color='red', label='SELL' , marker='v',s=80)
         plt.plot(tickerDf['Close'],label='Price',alpha = 0.5)
...
def ShowMAOnCandle(tickerDf,TickerSymbol):   
    ...
    myrange=range(1,101)
    flen=st.select_slider ("Choose your fast Moving Average Length",options=myrange,value=20, key='3')    
    slen=st.select_slider ("Choose your slow Moving Average Length",options=myrange,value=50, key='4')    
    
    chkShowMASignalOnCandle= st.checkbox("Show Buy & Sell Signal " , key='2')
...
def ShowMA(tickerDf,TickerSymbol):
    ...
    myrange=range(1,101)
    flen=st.select_slider ("Choose your fast Moving Average Length",options=myrange,value=20,key='5')    
    slen=st.select_slider ("Choose your slow Moving Average Length",options=myrange,value=50,key='6')    
    
    
    chkShowMASignal= st.checkbox("Show Buy & Sell Signal ",key='3')
...
chkRSI = st.sidebar.checkbox("Show RSI Indicator")
chkMFT= st.sidebar.checkbox("Show MFT Indicator")
chkMACD = st.sidebar.checkbox("Show MACD Indicator")
...
if ChkCandleStock  or ChkInfo or ChkData or chkBB or chkMFT or chkMA or chkPredict or ChkVol or chkRSI:    
     if Market=='Nazdaq':
       MarketIndex=1
     elif Market=='Crypto':
        MarketIndex=0
     ...
     features = Stocks('1d',start_date,end_date,MarketIndex,TickerSymbol,CompleteSymbol)
     try:            
        with st.spinner('Extracting Features... '):
             time.sleep(1)        
             temp=features.get_stock_data()
             tickerData= temp[0]
             tickerDf= temp[1]
...
# streamlit run "D:\University Shamsipour 1398\992-AI\Final Project\PricePrediction\Final.py"