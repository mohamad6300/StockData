...
path = 'D:/University Shamsipour 1398/992-AI/Final Project/PricePrediction'
Currency='USD'
...
#Begin -------------------------------------------------------------Change Background color dosent work 
st.markdown("""
<style>
body {
    color: #fff;
...
#0A3648
#13393E
#end -------------------------------------------------------------Change Background color dosent work 
...
if ChkCandleStock  or ChkInfo or ChkData or chkBB or chkMFT:    
     if Market=='Nazdaq':
        try:            
           with st.spinner('Extracting Features... '):
               time.sleep(1)
           # st.sidebar.info("Extracting Features..." )    
        ...
        # tickerDf.index = pd.to_datetime(tickerDf.index,errors='coerce').tz_lo‌​calize('US/Eastern')‌​.tz_convert('US/East‌​ern')`
        
        # tickerDf.index =pd.DatetimeIndex(pd.to_datetime(tickerDf.index )).tz_lo‌​calize('US/Eastern')‌​.tz_convert('US/East‌​ern')
          
            # tickerDf['Date'] = tickerDf.index
            # tickerDf['Date'] = pd.to_datetime(tickerDf['Date'])
            # tickerDf.dropna(subset=['Date'], inplace = True)
        ...
        # mask = (tickerDf['Date'] >= pd.to_datetime(start_date)) & (tickerDf['Date'] <= pd.to_datetime(end_date))
